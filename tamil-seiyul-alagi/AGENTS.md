...
- **Metre:** Coarse metre lives under [`src/metre/`](src/metre/) — `mod.rs` (`MetreType`), `prediction.rs` (heuristic head + dense boost), `fractions.rs`, `classical_checker.rs` (placeholder). Up to four hypotheses in `top_k_metre_hypotheses`; `parse_features` on `ParseResult`. See [`PARSE_FEATURES.md`](PARSE_FEATURES.md) and [`TRAINING_PROCESS.md`](TRAINING_PROCESS.md).
