...
- **Metre:** Coarse metre under [`src/metre/`](src/metre/) — `mod.rs` (`MetreType`), `prediction.rs` (heuristic + dense boost), `ml_head.rs` (shipped hybrid logit + `metre_hybrid_weights.inc.rs`; regenerate with `cargo run --example fit_metre_hybrid_weights`), `fractions.rs`, `classical_checker.rs` (placeholder). **Design and failure modes:** [`METRE_PREDICTION.md`](METRE_PREDICTION.md). See also [`PARSE_FEATURES.md`](PARSE_FEATURES.md) and [`TRAINING_PROCESS.md`](TRAINING_PROCESS.md).
